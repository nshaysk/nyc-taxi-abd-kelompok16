...
with col1:
    fig1 = px.histogram(
        df_filtered.sample(min(50000, len(df_filtered))),
...
# ===== ROW 3: JAM SIBUK & WEEKDAY VS WEEKEND =====
st.markdown("---")
...
@st.cache_resource
def train_model():
    X = df_gold[['trip_distance', 'passenger_count', 'pickup_hour',
...
with col2:
    ...
    month = st.selectbox("Bulan", options=[3, 4], format_func=lambda x: 'Maret' if x == 3 else 'April')
...
st.markdown("---")
st.caption("Kelompok 16 | Azzahra, Nayla, Fiodora, Wan Nashwa | Sains Data ITERA 2026")